...
* *[files/--ZDOTDIR--/.zsh_plugins.txt, files/--ZDOTDIR--/.zsh_plugins.zsh]* Deferred 6 additional plugins via `kind:defer` (antidote uses `romkatv/zsh-defer`) to reduce synchronous work on the shell startup hot path. Previously only `zsh-history-substring-search`, `fast-syntax-highlighting`, and `zsh-autosuggestions` were deferred. Now also deferred: `lib/termsupport.zsh` (terminal title/CWD hooks — cosmetic), `plugins/eza` (aliases only), `plugins/git` (heaviest plugin at 431 lines, aliases only), `plugins/iterm2` (shell integration hooks — cosmetic for first prompt), `plugins/sudo` (ESC-ESC key binding — fires before first keypress via `zsh-defer`), `plugins/zbell` (long-command bell hooks). Cannot defer: `lib/functions.zsh` (needed synchronously by `misc.zsh`/`termsupport.zsh`), `lib/completion.zsh` (must precede `compinit`), `lib/correction.zsh` (`setopt correct_all`), `lib/key-bindings.zsh` (key bindings must be active from first keystroke), `lib/misc.zsh` (registers `url-quote-magic` ZLE hook), `zsh-completions` (fpath before `compinit`), `plugins/direnv` (hook must fire before first command). Added detailed comments to `plugins.txt` explaining the deferral decision for every entry.
...
* *[files/--ZDOTDIR--/.zshrc]* Added `mkdir -p "${XDG_CACHE_HOME}"` before the first cache write. When `delete_caches` removes `~/.cache`, all subsequent `>|` cache-write redirections failed silently — the `brew shellenv` cache was never written, so `fpath` never received `${HOMEBREW_PREFIX}/share/zsh/site-functions`, breaking brew completions and antidote plugins on the next shell start. The `mkdir -p` ensures the directory always exists before any write.
* *[scripts/software-updates-cron.sh]* Removed `load_zsh_configs` call — it sourced `.zshrc` which activated mise and installed `command_not_found_handler`. That handler returned 127 in the non-interactive cron environment, firing the ERR trap. The script only needs `.aliases` (which sources `.shellrc`); nothing from the interactive zsh config is required when running in cron.
* *[personal/dev/configs/crontab.txt, files/--HOME--/.aliases]* Fixed `run-all.sh` and `capture-prefs.sh` not found in cron despite correct PATH. Crontab treats `#` as part of a value on assignment lines (not as a comment), so the inline `# Needed for...` comment was appended to the last PATH directory name, making it an invalid path. Moved the explanation to a standalone `#` comment line above the `PATH=` assignment. Added a warning comment to both files to prevent recurrence.
* *[personal/dev/configs/crontab.txt, files/--HOME--/.aliases]* Expanded crontab `PATH` to include `/usr/local/bin`, `/usr/sbin`, `/sbin`, `${PERSONAL_BIN_DIR}`, and `${DOTFILES_DIR}/scripts` — previously only `/opt/homebrew/bin:/usr/bin:/bin`, causing `run-all.sh`, `capture-prefs.sh`, `regenerate_repo_aliases`, and standard utilities (`rm`, `ls`, `cat`, `sh`, `realpath`) to be not found in cron.
...
* *[.github/instructions/shell-scripting.instructions.md]* Updated `zsh_eval_context` guard docs and autoload skeleton to use `*file*` and explain the colon-vs-space difference between contexts.
* *[personal/dev/configs/crontab.txt, files/--HOME--/.aliases]* Fixed cron invocation: changed from `chronic /opt/homebrew/bin/zsh script.sh 2>&1 | tee` (tee outside chronic's scope — log never written on success) to `chronic /opt/homebrew/bin/zsh -c 'zsh script.sh 2>&1 | tee'` so chronic wraps the full pipeline. The inner `zsh` runs the script in a fresh process with no stale function inheritance.
...
* *[scripts/software-updates-cron.sh]* Consolidated macOS notifications: removed the mid-run outdated notification (which was immediately replaced by the final "done" notification). The final notification now includes the comma-separated outdated package list when present, using title "⚠️ Software Updates"; otherwise shows "✅ Software Updates Done". `outdated_flat` is declared at `main` scope so it is available at the end of the function.
* *[.github/instructions/shell-scripting.instructions.md]* Updated `compdef` registration guard docs and autoload skeleton to include `|| true`; documented why it is required.
...
* *[.github/instructions/git-config.instructions.md, .github/copilot-instructions.md]* Updated `git cc`/`git rfc` docs to document the `refs/tags` exclusion rule and why it is required.
