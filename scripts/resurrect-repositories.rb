...
def find_git_remotes(folder)
  ...
  if status.success?
    ...
    warn("Could not retrieve remotes using 'git config --get-regexp' for '#{folder}' (status: #{status.exitstatus}).")
...
def apply_filter(repos, filter)
  ...
  repos.select do |repo_item|
    ...
    find_and_replace_env_var(path_to_check).strip.match?(/#{filter}/i)
  end
end
...
def resurrect_each(repo, idx, total)
  ...
  unless status.success?
    warn("Failed to fetch all remotes and tags for repo '#{folder}'")
...
def verify_all(repositories, discovered_count, filter, ref_folder: nil)
...
if options[:generate]
  ...
  generated = repositories.map { |dir| generate_each(dir) }
  ...
  discovered_count = repositories.length
  repositories = apply_filter(repositories, filter)
  verify_all(repositories, discovered_count, filter, ref_folder: reference_folder)
end
